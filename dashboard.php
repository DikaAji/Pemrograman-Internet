...
if(isset($_POST['nama_folder']) && isset($_POST['description'])){
	...
	if(count($_SESSION['error']) == 0 ){
		$sql = 'INSERT INTO folder (nama, description, user_id) VALUES ("'._e($_POST['nama_folder']).'", "'._e($_POST['description']).'", '.$_SESSION['login'].')';
...
<div class="panel panel-default">
	<div class="panel-body" id="files">
			<?php
			$query = mysqli_query($DB, 'SELECT * FROM file WHERE folder_id IN (SELECT id FROM folder WHERE user_id = '.$user->id.') '.($folder==0?'':'AND folder_id = '.$folder).' AND `type`="'._e($type).'"');
